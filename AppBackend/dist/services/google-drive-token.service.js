...
exports.GoogleDriveTokenService = void 0;
...
class GoogleDriveTokenService {
    ...
    static async saveTokens(userId, tokens) {
        const query = `
    ...
    static async updateAccessToken(userId, accessToken, expiryDate) {
        const query = `
    ...
    `;
        try {
            await database_1.pool.query(query, [userId, accessToken, expiryDate]);
    ...
    static async getUsersWithTokens() {
        const query = `
    ...
    `;
        try {
            const result = await database_1.pool.query(query, [fiveMinutesFromNow]);
            return result.rows.map(row => row.user_id);
        }
        catch (error) {
...
exports.GoogleDriveTokenService = GoogleDriveTokenService;
GoogleDriveTokenService.createTableIfNotExists().catch(console.error);
//# sourceMappingURL=google-drive-token.service.js.map